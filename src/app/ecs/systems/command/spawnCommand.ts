...
import {Component, HOST_HIDDEN_TYPE} from "../../component";
...
export class SpawnCommandKind implements CommandKind {
    ...
    stripClient(command: SpawnCommand): Command[] {
        const hostHidden = command.data.storages[HOST_HIDDEN_TYPE] as FlagEcsStorageSerialzed;

        const entityFilter = (x: number) => hostHidden === undefined || hostHidden.indexOf(x) < 0;
